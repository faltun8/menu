...
function displayCategoryItems(){
    ...
    menu.forEach(element => {
        const foodIndex = categories.indexOf(element.category);
    ...
    categories.forEach(element => {
        categoryContent += `<button type="button" class="filter-btn" data-id="${element}">${element}</button>`;
    });
    ...
    filterButtons.forEach(item => {
        ...
        item.addEventListener("click", function(element){
            const selectedCategoryName = element.target.getAttribute("data-id");
            console.log("2");
